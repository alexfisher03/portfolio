...
export default function ProjectItem({ title, description, link, imageUrl, altUrl, technologies }: Props) {
  ...
  return (
    <motion.li variants={itemVariants} className="group group-hover/projects:opacity-20 hover:opacity-100 transition-opacity duration-200 w-full">
      <Wrapper
        {...wrapperProps}
        className={`relative flex flex-col overflow-hidden rounded-[24px] border border-white/15 bg-black/40 backdrop-blur-sm shadow-[0_10px_20px_0_rgba(0,0,0,0.35)] shadow-red-500/10 transition-shadow ease-linear duration-200 ${link ? "" : "cursor-default"}`}
      >
        ...
        <div className="relative z-20 border-t border-white/10 px-4 py-3 sm:px-6 sm:py-4">
          <div className="rounded-sm transition-colors duration-150">
            <div className="flex items-start gap-1.5">
              <h3 className="min-w-0 flex-1 text-balance text-base font-semibold leading-normal text-white sm:text-lg group-hover:text-[#ff3232] transition-colors duration-200">
                {title}
              </h3>
              {link ? (
            ...
            <p className="mt-1 text-sm leading-relaxed text-white/75 line-clamp-3">{description}</p>
